...
export function ProjectionHub({ initialInvestment, initialRoi, profileTargets }: ProjectionHubProps) {
  ...
  const calculations = useMemo(() => {
    ...
    let personalROI = 0;
    if (isCompounding) {
      personalROI = months > 0 ? investmentAmount * (Math.pow(1 + roiRate, months) - 1) : 0;
    } else {
      personalROI = investmentAmount * roiRate * months;
    }
    const depositFee = investmentAmount * 0.05;
    const netPersonal = investmentAmount - depositFee + personalROI;

    // 2. Team Growth
    const levels = [];
    let currentLevelCount = l1Count;
    let totalTeamEarnings = 0;
    let totalNetworkSize = 0;
    let totalNetworkInvestment = 0;

    for (let i = 0; i < 15; i++) {
      const actualLevelCount = isManualLevels ? (levelCounts[i] || 0) : currentLevelCount;
      const levelInvestment = actualLevelCount * avgNetworkInvestment;
      ...
      const levelCommission = isLocked ? 0 : (levelInvestment * roiRate) * COMMISSION_RATES[i] * months; // Commission on their ROI
      
      levels.push({
        level: `L${i + 1}`,
        count: actualLevelCount,
      ...
      totalTeamEarnings += levelCommission;
      totalNetworkSize += actualLevelCount;
      totalNetworkInvestment += levelInvestment;
      
      if (!isManualLevels) {
        currentLevelCount = Math.floor(currentLevelCount * recruitmentRate);
      }
      
      if (!isManualLevels && (currentLevelCount === 0 || currentLevelCount > 1000000)) {
        // Continue but with 0 count for rest of levels if duplication stops
      }
    }

    const grandTotal = personalROI + totalTeamEarnings;
  ...
  }, [investmentAmount, avgNetworkInvestment, monthlyRoiRate, months, isCompounding, l1Count, recruitmentRate, isManualLevels, levelCounts]);
  ...
  const growthData = useMemo(() => {
    ...
    for (let i = 0; i <= months; i++) {
      const personal = isCompounding 
        ? investmentAmount * Math.pow(1 + roiRate, i) 
        : investmentAmount + (investmentAmount * roiRate * i);
      
      data.push({
        month: `M${i}`,
        Personal: parseFloat(personal.toFixed(2)),
        Total: parseFloat((personal + (calculations.totalTeamEarnings * (i / months))).toFixed(2))
      });
    }
    return data;
  }, [investmentAmount, monthlyRoiRate, months, isCompounding, calculations.totalTeamEarnings]);
        ...
        <Card className="xl:col-span-4 v56-glass premium-border h-fit sticky top-24">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <Slider value={[months]} onValueChange={(val) => setMonths(val[0])} min={0} max={60} step={1} />
            </div>
            ...
            <div className="pt-4 border-t border-white/5 flex items-center justify-between">
              <div className="space-y-1">
                <Label className="flex items-center gap-2 font-bold uppercase tracking-wider text-xs">
                  <Zap className="h-3 w-3 text-primary" />
                  Compound ROI
                </Label>
                <p className="text-[10px] text-muted-foreground">Reinvest 10% monthly returns</p>
              </div>
